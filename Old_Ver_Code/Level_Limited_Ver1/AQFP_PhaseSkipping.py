    ...
    if version:
        ...
        with open('../../temp.txt', 'w') as temp:
            ...
            for n in ntk.netlist:
                ...
                if n.depth == L_low or n.depth == L_high:
                    temp.write(f"D_{n.name} = {n.depth}\n")

                # Equation (2) for netlist gates
                elif L_low < n.depth < L_high:
                    if len(n.splitter_out) == 1:
                    ...
                    # fix all non-target gates' depth
                    temp.write(f"D_{n.name} = {n.depth}\n")

            # Equation (2) for splitters
            for s in ntk.splitters:
                if L_low < s.depth < L_high:
                    for g in s.fanouts:
                        if(g.gate_type!="splitter"):
                    ...
                    temp.write(f"D_{s.name} = {s.depth}\n")
            # Equation (8)
            for i in range(L_low, L_high + 1):
                temp.write(f"W_{i} >= 0\n")
                width_constraint = " + ".join([
                    f"X_{n.name}_{i}"
                    for n in ntk.netlist + ntk.splitters
                    if L_low <= n.depth <= L_high
                ])
            ...
            for n in ntk.netlist + ntk.splitters:
                if L_low <= n.depth <= L_high:
                    level_constraint = " + ".join([f"X_{n.name}_{i}" for i in range(L_low, L_high + 1)])
                    temp.write(f"{level_constraint} = 1\n")
            # Equation (7)
                    level_assignment = " + ".join([f"{i} X_{n.name}_{i}" for i in range(L_low, L_high + 1)])
                    temp.write(f"{level_assignment} - D_{n.name} = 0\n")
            # Equation (9)
            for i in range(min(2, L_low), L_high + 1):
